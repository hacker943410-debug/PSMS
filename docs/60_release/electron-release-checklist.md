...
- PostgreSQL 운영 DB 후보는
  `docs/60_release/postgresql-credential-cleanup-rehearsal-profile.md` PASS evidence가
  있어야 한다. SQLite-only Electron local release는 `N/A-SQLite-only`로 기록한다.
- PostgreSQL profile/client 추가 전에는
  `docs/60_release/postgresql-prisma-profile-preflight.md`의 SQLite runtime 분리 원칙을
  통과해야 한다.
...
4. credential compensation failure cleanup runbook 기준 limbo token scan 결과를
   기록한다.
5. PostgreSQL 운영 DB 후보는 credential cleanup rehearsal profile 결과를 기록한다.
6. PostgreSQL 운영 DB 후보는 `pnpm pg:profile:preflight`와
   `pnpm pg:profile:require-readiness` 결과를 기록한다. SQLite-only Electron 후보는
   readiness를 `N/A-SQLite-only`로 명시한다.
7. production reverse proxy, CDN, APM, error reporting의 query/body/header/cookie
   scrubber는 자동 검증 밖의 수동 release gate로 기록한다.
8. credential cleanup/release evidence artifact는
   `docs/60_release/credential-cleanup-release-evidence-template.md`의 naming, JSON shape,
   forbidden field checklist를 따른다.
