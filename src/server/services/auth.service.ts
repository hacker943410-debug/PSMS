...
import type { LoginInput } from "@/server/validation/auth.validation";
...
export async function loginWithCredentials(
  ...
  if (!canAttemptLogin(input.email, metadata.ipAddress)) {
  ...
  const user = await findUserForLoginByEmail(prisma, input.email);
  ...
  if (!user || !passwordMatches || !userAllowed) {
    recordFailedLogin(input.email, metadata.ipAddress);
  ...
  const loginSession = await prisma.$transaction(async (tx) => {
    const freshUser = await findUserForLoginByEmail(tx, input.email);
  ...
  clearLoginFailures(input.email, metadata.ipAddress);
